...
export async function GET() {
  try {
    const res = await adminFetch(`${backend}/admin/upstream/models`, { cache: "no-store" });
    return forwardJson(res, "读取上游模型目录失败。");
  } catch {
    return NextResponse.json({ error: "读取上游模型目录失败。" }, { status: 500 });
  }
}
