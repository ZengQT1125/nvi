...
export async function POST(
  ...
  try {
    ...
    const res = await adminFetch(`${backend}/admin/master-keys/${id}/rotate`, {
      method: 'POST',
      cache: 'no-store',
    });

    return forwardJson(res, '轮换 Master Key 失败。');
  } catch {
    return NextResponse.json({ error: '轮换 Master Key 失败。' }, { status: 500 });
  }
}
