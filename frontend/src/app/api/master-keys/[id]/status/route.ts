...
export async function PATCH(
  ...
  try {
    ...
    const res = await adminFetch(`${backend}/admin/master-keys/${id}/status`, {
      method: 'PATCH',
    ...
    return forwardJson(res, '更新 Master Key 状态失败。');
  } catch {
    return NextResponse.json({ error: '更新 Master Key 状态失败。' }, { status: 500 });
  }
}
