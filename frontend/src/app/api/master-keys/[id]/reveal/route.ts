...
export async function POST(
  ...
  try {
    ...
    const res = await adminFetch(`${backend}/admin/master-keys/${id}/reveal`, {
      method: 'POST',
      cache: 'no-store',
    });

    return forwardJson(res, '读取 Master Key 明文失败。');
  } catch {
    return NextResponse.json({ error: '读取 Master Key 明文失败。' }, { status: 500 });
  }
}
